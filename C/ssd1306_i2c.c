...
//  unsigned char count=0;
  ...
  OLED_ClearLint(2,4);
  OLED_DrawPartBMP(0,2,128,4,BMP,TEMPERATURE_TYPE);
...
{
//  char STR_IP[23] = "IP:";
  strcpy(IPSource,GetIpAddress());						//Get the IP address of the device
//  strcat(STR_IP,IPSource);
  OLED_ClearLint(2,4);
  OLED_DrawPartBMP(0,2,128,4,BMP,4);
//  OLED_ShowString(0,3,strcat(STR_IP,IPSource),8);       //Send the IP address to the lower machine
//  OLED_ShowString(0,3,STR_IP,8);       //Send the IP address to the lower machine
  OLED_ShowString(12,3,IPSource,8);       //Send the IP address to the lower machine
}
...
void LCD_DisplayHostname(void)
{
  ...
  OLED_DrawPartBMP(0,0,128,2,BMP,5);
  OLED_ShowString(0,0,hostname,8);                      //Send the hostname to the lower machine
}
...
*/
void LCD_Display(unsigned short int count, unsigned short int sleeptime)
{
  switch(count)
  {
    case 0:
      OLED_DrawBMP(0,0,128,4,BMP,6);
      sleep(sleeptime);
    break;
    case 1:
      OLED_Clear();
...
char* GetIpAddress(void)
{
    ...
    if (IPADDRESS_TYPE == ETH0_ADDRESS)
    {
      ...
      count=ioctl(fd, SIOCGIFADDR, &ifr);
      close(fd);
      if(count==0)
      {
